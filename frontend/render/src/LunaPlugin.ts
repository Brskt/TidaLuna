...
export class LunaPlugin {
	...
	static {
		// TidaLunar: window.close is handled by CEF lifecycle, not IPC.
	...
	private startReloadLoop() {
		...
		const reloadLoop = async () => {
			// Fail quietly
			await this.loadExports().catch((e) => console.warn("[@luna/core] loadExports failed:", e));
	...
	private async fetchPackage(): Promise<boolean> {
		try {
			...
			console.warn("[@luna/core] Plugin operation error:", e);
		} finally {
